...
dash.override(Builder.prototype, {
    ...
    clone: function clone() {
        ...
        cloned._single = dash.override({}, this._single || {});
        cloned._statements = dash.override([], this._statements || []);
        cloned._debug = this._debug;

        // `_option` is assigned by the `Interface` mixin.
        if (this._options) {
            cloned._options = dash.override({}, this._options);
        }
    ...
    join: function join(table, first) {
        ...
        if (typeof first === 'function') {
            ...
            if (arguments.length > 1) {
                join.on.apply(join, dash.toArray(arguments).slice(1));
            }
        }
    ...
    where: function where(column, operator, value) {
        ...
        if (dash.isObject(column) && !(column instanceof _raw2['default'])) return this._objectWhere(column);
    ...
    orWhere: function orWhere() {
        this._bool('or');
        var obj = arguments[0];
        if (dash.isObject(obj) && !(obj instanceof _raw2['default'])) {
    ...
    whereIn: function whereIn(column, values) {
        if (Array.isArray(values) && dash.isEmpty(values)) return this.where(this._not());
        this._statements.push({
            grouping: 'where',
            type: 'whereIn',
            column: column,
    ...
    union: function union(callbacks, wrap) {
        if (arguments.length === 1 || arguments.length === 2 && dash.isBoolean(wrap)) {
            ...
            callbacks = dash.toArray(arguments).slice(0, arguments.length - 1);
            wrap = arguments[arguments.length - 1];
            if (!dash.isBoolean(wrap)) {
                callbacks.push(wrap);
                wrap = false;
            }
    ...
    insert: function insert(values, returning) {
        this._method = 'insert';
        if (!dash.isEmpty(returning)) this.returning(returning);
    ...
    update: function update(values, returning) {
        ...
        if (!dash.isEmpty(ret)) this.returning(ret);
    ...
    modify: function modify(callback) {
        callback.apply(this, [this].concat(dash.tail(dash.toArray(arguments))));
        return this;
    },
