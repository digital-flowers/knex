...
function QueryCompiler(client, builder) {
    ...
    this.grouped = dash.groupBy(builder._statements, 'grouping');
    this.formatter = client.formatter();
}

var components = ['columns', 'join', 'where', 'union', 'group', 'having', 'order', 'limit', 'offset', 'lock'];

dash.override(QueryCompiler.prototype, {
    ...
    toSQL: function toSQL(method, tz) {
        ...
        var defaults = {
            method: method,
            options: (this.options || []).reduce(dash.override, {}),
            timeout: this.timeout,
            cancelOnTimeout: this.cancelOnTimeout,
            bindings: this.formatter.bindings,
            __knexQueryUid: _nodeUuid2['default'].v4()
        };
        if (dash.isString(val)) {
            val = {sql: val};
        }
        ...
        return dash.override(defaults, val);
    },
    ...
    select: function select() {
        ...
        return dash.compact(statements).join(' ');
    },
    ...
    insert: function insert() {
        ...
        } else if (typeof insertValues === 'object' && dash.isEmpty(insertValues)) {
    ...
    whereBetween: function whereBetween(statement) {
        return this.formatter.wrap(statement.column) + ' ' + this._not(statement, 'between') + ' ' + (statement.value || []).map(this.formatter.parameter.bind(this.formatter)).join(' and ');
    },
    ...
    _prepUpdate: function _prepUpdate(data) {
        data = dash.omitBy(data, undefined);
        var vals = [];
