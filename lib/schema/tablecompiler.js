...
function TableCompiler(client, tableBuilder) {
    ...
    this.grouped = dash.groupBy(tableBuilder._statements, 'grouping');
...
TableCompiler.prototype.getColumnTypes = function (columns) {
    return columns.map(dash.first).reduce(function (memo, column) {
...
TableCompiler.prototype.columnQueries = function (columns) {
    var queries = columns.map(dash.tail).reduce(function (memo, column) {
        if (!dash.isEmpty(column)) return memo.concat(column);
        return memo;
    }, []);
